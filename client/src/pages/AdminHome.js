...
const AdminHome = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await axios.get('https://vanamalaclinicserver.onrender.com/announcements');
  ...
  const fetchServices = async () => {
    try {
      const response = await axios.get('https://vanamalaclinicserver.onrender.com/services');
  ...
  const fetchLinks = async () => {
    try {
      const response = await axios.get('https://vanamalaclinicserver.onrender.com/links');
  ...
  const fetchTiming = async () => {
    try {
      const response = await axios.get('https://vanamalaclinicserver.onrender.com/timings');
  ...
  const fetchAbout = async () => {
    try {
      const response = await axios.get('https://vanamalaclinicserver.onrender.com/about');
  ...
  const handleAddItem = async (section) => {
    ...
    try {
      ...
      const response = await axios.post(`https://vanamalaclinicserver.onrender.com/${endpoint}`, { text: item });
  ...
  const handleRemoveItem = async (section, id) => {
    try {
      const endpoint = section === 'announcements' ? 'announcements' : section === 'links' ? 'links' : 'services';
      await axios.delete(`https://vanamalaclinicserver.onrender.com/${endpoint}/${id}`);
      if (section === 'announcements') {
  ...
  const handleSaveChanges = async (section, id, text) => {
    try {
      const endpoint = section === 'announcements' ? 'announcements' : section === 'links' ? 'links' : 'services';
      await axios.put(`https://vanamalaclinicserver.onrender.com/${endpoint}/${id}`, { text });
    } catch (error) {
  ...
  const handleSaveText = async (section) => {
    try {
      ...
      await axios.put(`https://vanamalaclinicserver.onrender.com/${endpoint}`, { text });
    ...
    <Layout className="layout min-h-screen flex flex-col">
      <Header className="bg-gradient-to-r from-teal-400 to-teal-600 text-white shadow-md">
        <div className="flex justify-between items-center max-w-6xl mx-auto">
          <div className="text-l font-bold">Vanamala Clinic</div>
